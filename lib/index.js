...
api_1.default
    .runImport("Budget 2", () => __awaiter(void 0, void 0, void 0, function* () {
    ...
    const uploadedcategories = yield helpers_1.uploadCategories(categories);
    const payees = yield api_1.default.getPayees();
    const actualCategoriesWithId = yield api_1.default.getCategories();
    const transactions = helpers_1.getTransactions(jsonTransactions, categories, actualCategoriesWithId, accountsWithId, payees);
    const transactionsGroupedByAccount = helpers_1.transactionsByAccount(transactions);
    const results = yield helpers_1.uploadTransactions(transactionsGroupedByAccount);
    console.log(results);
}))
    .then(() => {
